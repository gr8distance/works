class SelectionsController < ApplicationController
  ...
  def new
    @selection = Selection.new
		@types = %W{iPhoneケース Androidケース シール SDカード 充電器 バッテリー ケーブル}
		@staffs = {};current_user.staffs.each{|s| @staffs["#{s.name}"] = s.id}
  ...
  private
    ...
    def selection_params
      params.require(:selection).permit(:price, :user_id, :staff_id, :split, :types)
    end
end
